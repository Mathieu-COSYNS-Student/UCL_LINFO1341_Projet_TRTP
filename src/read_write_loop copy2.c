...
{
    char read_buffer[MAX_SEGMENT_SIZE];
    int ret = fread(0, buffer, sizeof(buffer));
    if (ret < 0) {
        perror("couldn't read from stdin");
    }
    ssize_t written = write(socket, read_buffer, MAX_SEGMENT_SIZE));
    fflush(0);
}

int receive_messages(int socket, char* bufferReceived)
{
    ssize_t amount_read = read(socket, bufferReceived, MAX_SEGMENT_SIZE);
    if (amount_read == -1) {
